...
**GSLP gotcha:** the GSLP exes ship with most Nick/Tapani patches PRE-BAKED (coloured attributes, 9 subs, regen fixes, load-all-players, unprotected contracts by GS; HiddenAttributes columns baked by this fork via `CM0102Patcher tools/gslp_hiddencols.py`). Their `ConfigLines` write these as `false` — re-applying loader patches on top of the modified exe corrupts it (e.g. the loader-applied HiddenAttributes cave lands at base+RVA in GS's live runtime BSS → save-load/squad crash). Only byte-verified-clean patches are enabled (`HideNonPublicBids` + the `NoForeignRestrictionsForAll.patch` file, managed by `SetupDatabase`).
